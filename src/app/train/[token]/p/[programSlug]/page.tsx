...
export default function ProgramDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch trainee data
        const traineeRes = await fetch(`/api/trainee?token=${token}`);
        ...
        setError('This training link is invalid or the program was not found.');
      } finally {
        setLoading(false);
      }
    }
    ...
    if (lockedPrerequisite && trainee) {
      return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
          <div className="max-w-md bg-white rounded-lg border border-slate-200 p-8 text-center">
            <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <svg className="w-8 h-8 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
              </svg>
            </div>
            <h1 className="text-xl font-semibold text-slate-900 mb-2">Course Locked</h1>
            <p className="text-slate-600 mb-6">
              Pass {lockedPrerequisite.title} to unlock this course.
            </p>
            {lockedPrerequisite.enrolled ? (
              <Link
                href={`/train/${token}/p/${lockedPrerequisite.slug}`}
                className="inline-block px-6 py-3 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-colors"
              >
                Go to Prerequisite
              </Link>
            ) : (
              <p className="text-sm text-slate-500">Ask your administrator to assign the prerequisite course.</p>
            )}
          </div>
        </div>
      );
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-6 mb-8">
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-sm">
            ...
            {!allComplete && nextSection && (
              <Link
                href={`/train/${token}/p/${programSlug}/${nextSection.slug}`}
            ...
            {sectionProgress.map((section, index) => {
              ...
              return (
                <div key={section.id} className="p-4 flex items-center justify-between">
                  ...
                  {isAccessible ? (
                    <Link
                      href={`/train/${token}/p/${programSlug}/${section.slug}`}
        ...
        <div className={`mt-8 rounded-lg border p-6 ${
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              {allComplete ? (
                <Link
                  href={`/train/${token}/p/${programSlug}/assessment`}
